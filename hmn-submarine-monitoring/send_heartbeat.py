...
Formato personalizado conforme solicitado
"""
...
def format_discord_message(agents, tasks, swarm, history):
    ...
    for agent_id, agent_data in agents.items():
        ...
        if status == "active":
            ...
            idle_agents.append((agent_id, agent_data))
    ...
    if active_agents:
        ...
        for agent_id, agent_data in active_agents:
            ...
            msg += f"- **{agent_type}** (`{truncate_id(agent_id)}`)\n"
...
def send_to_discord(message):
    try:
        ...
        # Use curl to send to Discord webhook
        # Escape message for JSON
        import json
        payload = json.dumps({"content": message})
        
        cmd = f'curl -s -X POST -H "Content-Type: application/json" -d \'{payload}\' "{webhook_url}"'
        ...
        if result.returncode == 0:
            ...
            if "error" in result.stdout.lower():
                print(f"✗ Discord error: {result.stdout}")
                return False
            print(f"✓ Mensagem enviada para Discord webhook (canal alvo: {DISCORD_CHANNEL})")
            return True
        else:
...
def main():
    ...
    # Get swarm info - need to find the swarm ID
    swarms = swarm_data.get("swarms", {})
    swarm_info = {}
    if swarms:
        # Get first swarm (should be only one)
        swarm_id = list(swarms.keys())[0]
        swarm_info = swarms[swarm_id]
